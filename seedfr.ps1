...
('SAMM'),('SSDF'),('SLSA'),('Auditabilité');

-- SAMM practices
INSERT INTO practices(model_id, code, name)
SELECT m.id, x.code, x.name
FROM maturity_models m
JOIN (SELECT 'GOV.1' AS code, 'Politique de gouvernance' AS name
      UNION ALL SELECT 'TST.1','Stratégie de test'
      UNION ALL SELECT 'OPS.1','Gestion des opérations et des incidents') x
ON m.name='SAMM'
...
SELECT m.id, x.code, x.name
FROM maturity_models m
JOIN (SELECT 'PS.3' AS code, 'Construction de logiciels sécurisés' AS name
      UNION ALL SELECT 'RV.1','Examen des exigences de sécurité' AS name) x
ON m.name='SSDF'
LEFT JOIN practices p ON p.model_id=m.id AND p.code=x.code
WHERE p.id IS NULL;
...
SELECT m.id, x.code, x.name
FROM maturity_models m
JOIN (SELECT 'BUILD.L2' AS code, 'Intégrité de compilation L2' AS name
...
SELECT m.id, x.code, x.name
FROM maturity_models m
JOIN (SELECT 'EVID.1' AS code, 'Disponibilité des preuves' AS name
      UNION ALL SELECT 'TRACE.1','Traçabilité des changements' AS name) x
ON m.name='Auditabilité'
LEFT JOIN practices p ON p.model_id=m.id AND p.code=x.code
WHERE p.id IS NULL;
'@
...
for ($ci = 1; $ci -le $Companies; $ci++) {
  ...
  $cContact = "Julien.Savard$ci@company.com"
...
"@
  ...
  for ($ti = 1; $ti -le $TeamsPerCompany; $ti++) {
    $tName = "Équipe produit-$ti-$companyserial"
    $tNick = "t$ti"
    $tPurpose = "Service $ti"
    $tDesc = "Équipe de démonstration $ti pour l\'entreprise $cName"
...
($aid,'Fréquence de déploiement par semaine',$deploys,'deploys/week',NOW()),
($aid,'Délai pour les changements',$leadP50,'hours',NOW()),
($aid,'Taux d\'échec de changement',$cfr,'%',NOW()),
($aid,'Temps moyen de récupération',$mttrH,'hours',NOW()),
($aid,'Taux Eac par Iac',$eacr,'%',NOW())
"@
...
FROM practices p JOIN maturity_models m ON p.model_id=m.id
WHERE m.name='Auditabilité' AND p.code='EVID.1';
"@
